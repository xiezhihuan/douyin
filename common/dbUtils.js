...
class DbUtils {
    ...
    async updateOneByField(collName, fieldObj, updateObj) {
        const db = await _connect();
        return new Promise((resolve, reject) => {
            db.collection(collName).updateOne(fieldObj, {
                $set: {
                    ...updateObj,
                    delete: 0
                }
    ...
    async find(collName,whereObj={},limit=0,skip=0,sortObj={}){
        const db = await _connect();
        return new Promise((resolve,reject)=>{
    ...
    /*根据id查找到该数据并进行修改*/
    async updateOneById(collName,id,updateObj){
        const db = await _connect();
        return new Promise((resolve,reject)=>{
